...
| Advanced P/D path unavailable | advanced availability degradation | fall back to colocated only when that route is healthy; otherwise fail | requested/selected route, fallback reason |
...
`Advanced P/D -> colocated` is an optional inference-route fallback, not
authorization to retry a tool with broader privilege. Tool and agent failures
always fail closed.
...
are `NA`. Do not describe a fallback as successful Advanced P/D execution.
