...
CONTAINER="${LAB03_DECODE_CONTAINER:-lab03-decode}"
PORT="${DECODE_PORT:-8102}"
...
docker run --rm -d --name "${CONTAINER}" --gpus 'device=1' --shm-size 2g \
...
echo "Started decode worker ${CONTAINER}; verify readiness before routing traffic."
